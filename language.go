...
// LanguageMetadata holds the metadata associated with a language.
//
...
// ABIVersion returns the ABI version number for this language. This version number is used
// to ensure that languages were generated by a compatible version of Tree-sitter.
func (l *Language) ABIVersion() int {
	return int(C.ts_language_abi_version(l.c()))
}

// Version returns the ABI version number for this language.
// This version number is used  o ensure that languages were generated by a compatible version of Tree-sitter.
func (l *Language) Metadata() LanguageMetadata {
	x := C.ts_language_metadata(l.c())

	return LanguageMetadata{
		MajorVersion: byte(x.major_version),
		MinorVersion: byte(x.minor_version),
		PatchVersion: byte(x.patch_version),
	}
}

func (l *Language) Name() string {
	return C.GoString(C.ts_language_name(l.c()))
}
